...
**충돌할 때 뼈가 툭툭 튀던 현상을 잡았습니다.** 특히 체인 끝쪽 뼈에서
심하게 나타나던 문제입니다. 원인이 네 가지였고, 전부 수정했습니다.

### 밀어내기가 속도로 새던 문제

충돌로 뼈를 표면 밖으로 밀어내면, 그 밀어낸 거리가 **뼈가 스스로 이동한
것처럼** 속도로 환산되고 있었습니다. 서브스텝 하나를 기준으로 나누다 보니
아주 작은 보정도 큰 속도로 부풀었습니다.

증상이 뚜렷했습니다 — Substeps를 **올릴수록 오히려 더 튀었습니다**
(4 → 8 → 16 → 32 으로 갈수록 악화). 이제 밀어내기는 위치만 고치고,
운동량은 반발·마찰이 한 번만 처리합니다.

부수 효과로 **Friction이 제 역할을 되찾았습니다.** 예전에는 Friction을
0으로 두면 시뮬레이션이 20배 거칠어졌는데, 그건 마찰이 아니라 이 가짜
속도를 지워주고 있었기 때문입니다. 이제 Friction은 원하는 질감대로
쓰시면 됩니다.

### 킹크 완화가 충돌 해결을 되돌리던 문제

체인의 지그재그를 펴주는 안티킹크 보정이 충돌 처리 **뒤에** 실행되면서,
표면에 닿아 있는 뼈를 이웃 방향으로 섞어 **콜라이더 안쪽으로 다시**
밀어넣고 있었습니다. 다음 서브스텝이 또 밀어내고, 그게 반복되는 밀당이
바로 "툭툭"이었습니다.

이제 **접촉 중인 뼈는 건드리지 않습니다.** 공중에 떠 있는 뼈는 예전처럼
킹크 완화를 받습니다.

### 밀어내는 방향

깊이는 표면의 최근접점 기준으로 재면서, 미는 **방향**은 삼각형의 면법선을
쓰고 있어 둘이 어긋나 있었습니다. 면법선은 어느 삼각형이 가장 가까운지가
바뀌는 순간 계단처럼 튑니다 — 측정해 보니 한 스텝에 최대 22.5°까지
꺾였습니다(최근접점 기준은 1.1°). 콜라이더를 세분화해도 사라지지 않습니다.
면의 경계마다 생기는 문제라 면 크기와 무관하기 때문입니다.

이제 깊이와 방향을 같은 기준으로 계산합니다.

### 변형되는 콜라이더

캐릭터 메쉬처럼 **아마츄어나 셰이프키로 변형되는** 콜라이더가 가장 큰
문제였습니다. 이런 메쉬는 정점만 움직이고 오브젝트 자체는 제자리에 있어서,
시뮬레이션이 **표면이 멈춰 있다고 착각**했습니다. 그래서 밀어낸 자리에
표면이 또 들어오고, 다시 밀어내기를 반복했습니다.

이제 실제로 부딪힌 면이 지난 프레임 대비 얼마나 움직였는지 측정해서,
그 표면을 기준으로 접촉을 해결합니다.

### 그 외

- 애드온 환경설정 패널을 없앴습니다. 안에 있던 항목은 아무 동작도
  하지 않는 체크박스 하나와, 사용자가 만질 이유가 없는 내부
  안전값 하나뿐이었습니다. 후자는 코드 상수로 옮겼습니다.

> 💡 새로 생긴 설정은 없습니다. 기존 씬은 그대로 열리며, 움직이지 않는
> 콜라이더를 쓰는 씬은 결과가 동일합니다.
...
4. **안티폴드** — 모든 접촉 보정에는 예산이 있습니다(라운드당 뼈 길이의
   ...
   미끄러져 내려갑니다.
5. **안티킹크** — 접촉 해결 후, 체인 중간 뼈들의 방향을 이웃 뼈 평균
   방향으로 15%씩 완만히 혼합(Chain 모드만)합니다. 구 위에 드레이프된
   체인의 지그재그 꺽임이 몇 프레임 안에 자연스럽게 펴집니다.
   **표면에 닿아 있는 뼈는 제외**됩니다 — 충돌로 잡아준 자세를 다시
   흐트러뜨리지 않기 위해서입니다(v1.1.0).
...
1. v1.0.0 솔버 사용 확인 (안티폴드 + 안티킹크 내장)
